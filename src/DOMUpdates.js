...
export const domUpdates = {
  ...
  displayTripCards(traveler, allDestinations, cardType) {
    ...
    if (traveler[cardType].length > 0) {
      traveler[cardType].forEach((trip) => {
        ...
        let cardInfo = `<section class="card">
          <div class= "image-section">
            <img class= "trip-image" src= "${
              trip.currentDestination.image
            }" alt = "${trip.currentDestination.alt}">
          </div>
  ...
  displayTripQuote(tripWithDestination) {
    ...
    const totalLodgingCost =
      tripWithDestination.currentDestination.estimatedLodgingCostPerDay *
      (tripWithDestination.duration / 86400000);
    console.log(totalLodgingCost, "total lodging cost");
    const totalFlightCost =
      tripWithDestination.currentDestination.estimatedFlightCostPerPerson *
      tripWithDestination.travelers *
      2;
